...
Channel.prototype._init = function () {
    this.channel.on('connection', function (socket) {
        // acknowledge server version to the client
        socket.emit('serverVersion', require('../package.json').version);
...
Channel.prototype.unload = function () {
    socket.broadcast.to(this.channelID).emit('shouldDisconnect');
